...
def explore_get_coordinates() -> List[List[List[int]]]:
    ...
    rotated_xy: List[List[List[int]]] = [
        [int(temp[0]), rf.rotate_box_dot(x_cen=float(temp[1]), y_cen=float(
            temp[2]), width=float(temp[3]), height=float(temp[4]), theta=float(temp[5]))]
        for temp in temps
    ]
...
def explore_main_get_coordinates() -> List[List[int]]:
    ...
    rotated_xy: List[List[int]] = [
        rf.rotate_box_dot(x_cen=float(temp[0]), y_cen=float(temp[1]), width=float(
            temp[2]), height=float(temp[3]), theta=float(temp[4]))
        for temp in temps
    ]
    # print('회전한 값 x1, 회전한 값 y1, ..., x4, y4')
    # print(f'회전된 데이터 List : {rotated_xy}')
...
def sub_main():
    ...
    for img_fn in imgs_filename:
        ...
        coordinates = []
        # data ex) ['18935', [383, 2, 90, 2, 90, 139, 383, 138]], ['18932', [377, 327, 61, 364, 78, 506, 394, 469]]
        for data in rotated_xy:
            if number == data[0]:
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            for xy_list in rotated_xy:
                cv2.line(img=img, pt1=(xy_list[0], xy_list[1]), pt2=(
                    xy_list[2], xy_list[3]), color=(0, 200, 0), thickness=2, lineType=cv2.LINE_AA)
                cv2.line(img=img, pt1=(xy_list[2], xy_list[3]), pt2=(
                    xy_list[4], xy_list[5]), color=(0, 200, 0), thickness=2, lineType=cv2.LINE_AA)
                cv2.line(img=img, pt1=(xy_list[4], xy_list[5]), pt2=(
                    xy_list[6], xy_list[7]), color=(0, 200, 0), thickness=2, lineType=cv2.LINE_AA)
                cv2.line(img=img, pt1=(xy_list[6], xy_list[7]), pt2=(
                    xy_list[0], xy_list[1]), color=(0, 200, 0), thickness=2, lineType=cv2.LINE_AA)
